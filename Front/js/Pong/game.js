    ...
    async createTournament() {
        ...
        const runMatch = (left, right) => {
            return new Promise((resolve) => {
                ...
                const updateScores = () => {
                    if (UI.scoreLeft)
                        UI.scoreLeft.textContent = String(leftScore);
                    if (UI.scoreRight)
                        UI.scoreRight.textContent = String(rightScore);
                };
                updateScores();
        ...
        if (UI.playersArea)
            UI.playersArea.classList.add("hidden");
        if (UI.scoreLeft)
...
function startMatch() {
    var _a;
    (_a = UI.playButton) === null || _a === void 0 ? void 0 : _a.classList.add("hidden");
    UI.readyText.classList.remove("hidden");
    setTimeout(() => {
        UI.readyText.classList.add("hidden");
        UI.goText.classList.remove("hidden");
