...
export default async function statsRoutes(fastify: FastifyInstance) {
	...
	fastify.get("/:userId", { preHandler: [authentizer()] }, async (req, reply) => {
		const userId = Number((req.params as StatsParams).userId);
	...
	fastify.post("/", { preHandler: [authentizer()] }, async (req, reply) => {
		const { userId, gamesPlayed, gamesWon, matchTime } = req.body as StatsBody;

		const stats = await statsService.upsertStats(
			userId,
			gamesPlayed ?? 1,
			gamesWon ?? 0,
			matchTime ?? 0
