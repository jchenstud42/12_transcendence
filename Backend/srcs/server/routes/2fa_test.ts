...
export default async function twofaTestRoutes(fastify: FastifyInstance) {

	fastify.post<{ Params: { userId: number } }>('/generate-2fa-test/:userId', async (request, reply) => {
		try {
	...
	fastify.post<{ Params: { userId: number } }>('/verify-2fa/:userId', async (request, reply) => {
		try {
			const userId = Number(request.params.userId);
			const { code } = request.body as { code: string };

			if (!userId || !code) return reply.status(400).send({ error: "Missing parameters" });

			const result = twofa.complete2FA(userId, code);
			if (!result) return reply.status(401).send({ error: "Invalid 2FA code" });
