...
export class Match {
    constructor(isTournament, matchPlayer = null) {
        ...
        this.ballLoop = (now = performance.now()) => {
            if (!this.ball.active) {
                this.ballLoopRafId = null;
                return;
            }
        ...
        Match.pendingTimeouts.push(setTimeout(() => {
            ...
            Match.pendingTimeouts.push(setTimeout(() => {
                ...
                Match.pendingTimeouts.push(setTimeout(() => {
                    ...
                    this.ball.serve();
                    if (this.aiLeft)
                        this.aiLeft.oneSecondLoop(), this.aiLeft.showAiPredictions();
                    if (this.aiRight)
                        this.aiRight.oneSecondLoop(), this.aiRight.showAiPredictions();
                    this.last = performance.now();
                    this.ballLoopRafId = requestAnimationFrame(this.ballLoop);
    ...
    endMatch(winner) {
        ...
        saveMatchHistory(this.matchPlayer[0], this.matchPlayer[1], this.matchPlayer[0].point, this.matchPlayer[1].point, winner, this.matchStat.nbrOfBallHit, this.matchStat.nbrOfBallMissed, this.matchStat.matchTime);
        this.partialReset();
...
function showGameElements() {
    showMenu(PONG_UI.ball, PONG_UI.leftPaddle, PONG_UI.rightPaddle);
}
export async function saveMatchHistory(player1, player2, score1, score2, winner, nbrOfBallHit, nbrOfBallMissed, matchTime) {
...
async function savePlayerStats(userId, isWinner, matchTime, token) {
    try {
        const statsPayload = {
            userId,
            gamesPlayed: 1,
...
export async function saveStats(playerId, gamesWon, matchTime) {
    const token = localStorage.getItem("accessToken");
    if (!token)
        return;
    const payload = {
