...
import { hideMenu } from "./UI/UI_helpers.js";
...
const global_stats_menu = document.getElementById("global-stats-menu");
const stats_back_button = document.getElementById("stats-back-button");
const buttonGlobalStats = document.getElementById("stats-button");
        ...
        if (submit) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    alert(t("server_error_prefix") + " " + getServerErrorMessage(data === null || data === void 0 ? void 0 : data.error));
                }
            }
            catch (err) {
    ...
    logoutButton.addEventListener("click", async () => {
        try {
            ...
            if (res.ok) {
                ...
                alert(t("logout_error") + " " + getServerErrorMessage((err === null || err === void 0 ? void 0 : err.error) || (err === null || err === void 0 ? void 0 : err.data)));
            }
        }
        catch (err) {
            console.error(err);
...
async function fetchGlobalStats(userId) {
    try {
...
function renderMatchHistory(matches, stats) {
...
function renderMatchStat(matches, match, div, stats) {
    ...
    buttonStats.addEventListener("click", () => {
        var _a, _b;
        if (global_stats_menu === null || global_stats_menu === void 0 ? void 0 : global_stats_menu.classList.contains("hidden"))
        ...
        if (statsList) {
            statsList.innerHTML = `
					...
					<div>${"Balls hit"}: ${(_a = match.nbrOfBallHit) !== null && _a !== void 0 ? _a : 0}</div>
					<div>${"Balls missed"}: ${(_b = match.nbrOfBallMissed) !== null && _b !== void 0 ? _b : 0}</div>
...
;
function renderGlobalStats(matches, stats) {
    var _a, _b, _c;
    ...
    if (stats && stats.gamesPlayed > 0) {
        const gamesPlayed = (_a = stats.gamesPlayed) !== null && _a !== void 0 ? _a : 0;
        const gamesWon = (_b = stats.gamesWon) !== null && _b !== void 0 ? _b : 0;
    ...
    (_c = oldButton.parentNode) === null || _c === void 0 ? void 0 : _c.replaceChild(newButton, oldButton);
