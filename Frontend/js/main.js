...
function generateLoggedUserId() {
    const id = loggedUserCounter;
    loggedUserCounter++;
    if (loggedUserCounter >= 200)
...
function startMatch() {
    play_button === null || play_button === void 0 ? void 0 : play_button.classList.add("hidden");
    ready_text.classList.remove("hidden");
    pendingTimeouts.push(setTimeout(() => {
        ready_text.classList.add("hidden");
        go_text.classList.remove("hidden");
...
export function resetGameMenu() {
    ...
    if (score_left)
        score_left.textContent = "0";
    if (score_right)
        score_right.textContent = "0";
    hideMenu(OK_button, paddle_left, paddle_right, ready_text, go_text, playerName_container, increasePlayer_button, decreasePlayer_button, ball, aiCounter, players_area);
...
class Game {
    ...
    async createTournament() {
        ...
        const saveTournamentMatch = async (player1, player2, score1, score2, winner) => {
            const token = localStorage.getItem("accessToken");
            if (!token)
                return;
            const payload = {
        ...
        const runMatch = (left, right, onPlayClick) => {
            return new Promise((resolve) => {
                ...
                const updateScores = () => {
                    if (score_left)
                        score_left.textContent = String(leftScore);
                    if (score_right)
                        score_right.textContent = String(rightScore);
                };
                updateScores();
                ball.classList.add("hidden");
                play_button.classList.remove("hidden");
        ...
        if (players_area)
            players_area.classList.add("hidden");
        if (score_left)
            score_left.textContent = "0";
        if (score_right)
            score_right.textContent = "0";
